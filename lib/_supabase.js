...
async function normalizeHistoryRow(admin, row) {
  ...
  const linkedThumbUrl = thumbUrl || meta.thumbnail_fallback || (!isVideo ? (meta.previewUrl || null) : null);
  const item = {
    id: row.id,
    type: row.type,
    prompt: row.prompt || '',
...
function buildDesignPresetState(rows, customRows) {
  ...
  const customPresets = (customRows || []).map((row) => ({
    id: row.id,
...
async function fetchAccountData(admin, userId, options = {}) {
  ...
  const customPresets = await Promise.all(designState.customPresets.map(async (preset) => ({
    ...preset,
...
async function replaceDesignPresetState(admin, userId, state) {
  ...
  const nextRows = [];
  for (const preset of incomingCustom) {
    const presetId = preset && preset.id ? String(preset.id) : crypto.randomUUID();
    keepIds.add(presetId);
    let imagePath = preset.storagePath || null;
    if (!imagePath && preset.dataUrl) {
      const basePath = `${userId}/design-presets/${presetId}`;
...
function prepareHistoryRow(userId, rawItem) {
  ...
  const previewUrl = rawItem && (rawItem.thumbnailUrl || rawItem.url) ? (rawItem.thumbnailUrl || rawItem.url) : null;
  const meta = {
    clientId: itemId,
...
function normalizeHistoryRowFallback(row) {
  ...
  const isVideo = row.type === 'video';
  const hasLinkedMedia = !!(meta.originalUrl || meta.originalDownloadUrl || meta.previewUrl || meta.thumbnail_fallback || meta.placeholderUrl);
  const linkedThumbUrl = meta.thumbnail_fallback || (!isVideo ? (meta.previewUrl || null) : null);
  const item = {
    id: row.id,
    type: row.type,
    prompt: row.prompt || '',
